...
export function getHospitalIntelligence({ queue, sessions, ipdList, labOrders, charges, staff, patients }) {
  ...
  return {
    ...
    insights: [
      leakage.count > 0 && { type: 'warning', icon: '💰', title: `Revenue Leakage Detected`, body: `₹${leakage.total_leakage.toLocaleString()} potential loss across ${leakage.count} unbilled service(s)` },
      criticalQueue.length > 0 && { type: 'critical', icon: '🚨', title: `${criticalQueue.length} Critical Patient(s) in Queue`, body: `Immediate attention required` },
      dischargeCandidates.length > 0 && { type: 'info', icon: '🏥', title: `${dischargeCandidates.length} Patient(s) Ready for Discharge`, body: dischargeCandidates.map(d => d.hospital_patients?.full_name || 'Unknown').join(', ') },
      shiftOpt.understaffed.length > 0 && { type: 'warning', icon: '👥', title: `Understaffed Shifts`, body: `${shiftOpt.understaffed.join(', ')} shift(s) below minimum coverage` },
    ].filter(Boolean)
  };
}
